...
export default {
  ...
  methods: {
    ...
    deleteItem (item) {
      ...
      this.$http.delete('https://spnfantasyfunc.azurewebsites.net/api/players/1/' + item.id)
    },
    close () {
      this.dialog = false
      setTimeout(() => {
    ...
    sendFormData (item) {
      this.$http.post('https://spnfantasyfunc.azurewebsites.net/api/players/1/' + item.id, item)
    }
  }
}
</script>
