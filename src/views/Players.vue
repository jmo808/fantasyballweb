...
export default {
  ...
  methods: {
    ...
    deleteItem (item) {
      ...
      this.$http.delete('https://fantasyslofunc.azurewebsites.net/api/players/1/' + item.id)
    },
    close () {
      this.dialog = false
      setTimeout(() => {
    ...
    sendFormData (item) {
      this.$http.post('https://fantasyslofunc.azurewebsites.net/api/players/1/' + item.id, item)
    }
  }
}
</script>
